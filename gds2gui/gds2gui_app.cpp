#include "gds2gui_app.h"
#include "gds2gui_frame.h"

bool Gds2GuiApp::OnInit()
{
	Gds2GuiFrame *frame = new Gds2GuiFrame (_T("gds2gui"));
	frame->Show(TRUE);
	SetTopWindow(frame);
	return TRUE;
}

IMPLEMENT_APP(Gds2GuiApp)
...
void Gds2GuiApp::PrintUsage()
{
	printf("gds2gui  version %s\n", VERSION);
...
int main(int argc, char *argv[])
{
	...
	*/
	...
	if(iptr){
		class GDSParse *baseParser = new class GDSParse(config, process, generate_process);
		if(!baseParser->Parse(iptr)){
			if(generate_process){
				...
				if(!svg){
					class GDSParse_pov *Parser = new GDSParse_pov(baseParser);
					...
					class GDSParse_svg *Parser = new GDSParse_svg(baseParser);
